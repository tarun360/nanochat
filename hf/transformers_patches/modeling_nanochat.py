...
class NanoChatAttention(nn.Module):
    ...
    def __init__(self, config: NanoChatConfig, layer_idx: int):
        ...
        # Value embedding gate (ResFormer-style, on alternating layers)
        if getattr(config, 'use_value_embeddings', False) and _has_ve(layer_idx, config.num_hidden_layers):
            self.ve_gate_channels = getattr(config, 've_gate_channels', 32)
            self.ve_gate = nn.Linear(self.ve_gate_channels, config.num_key_value_heads, bias=False)

    def forward(
        self,
        hidden_states: torch.Tensor,
        value_embeddings: torch.Tensor | None = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if value_embeddings is not None and hasattr(self, 've_gate'):
            B, T = hidden_states.shape[:2]
            ve = value_embeddings.view(B, T, -1, self.head_dim).transpose(1, 2)  # (B, n_kv, T, D)
            gate = 2 * torch.sigmoid(self.ve_gate(hidden_states[..., :self.ve_gate_channels]))  # (B, T, n_kv)
            gate = gate.permute(0, 2, 1).unsqueeze(-1)  # (B, n_kv, T, 1)
            value_states = value_states + gate * ve
...
class NanoChatModel(NanoChatPreTrainedModel):
    def __init__(self, config: NanoChatConfig):
        ...
        # Per-layer learnable residual scaling and x0 skip connections
        if getattr(config, 'use_resid_lambdas', False):
            self.resid_lambdas = nn.Parameter(torch.ones(config.num_hidden_layers))
        if getattr(config, 'use_x0_lambdas', False):
            self.x0_lambdas = nn.Parameter(torch.full((config.num_hidden_layers,), 0.1))

        # Value embeddings (ResFormer-style): alternating layers, last layer always included
        if getattr(config, 'use_value_embeddings', False):
            head_dim = config.hidden_size // config.num_attention_heads
            kv_dim = config.num_key_value_heads * head_dim
            self.value_embeds = nn.ModuleDict({
                str(i): nn.Embedding(config.vocab_size, kv_dim)
                for i in range(config.num_hidden_layers)
                if _has_ve(i, config.num_hidden_layers)
            })
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Save initial normed embedding for x0 residual skip connections
        x0 = hidden_states if getattr(self.config, 'use_x0_lambdas', False) else None

        # Pre-compute value embeddings for all layers that need them
        ve_per_layer = {}
        if getattr(self.config, 'use_value_embeddings', False) and hasattr(self, 'value_embeds') and input_ids is not None:
            for layer_idx_str, ve_embed in self.value_embeds.items():
                ve_per_layer[int(layer_idx_str)] = ve_embed(input_ids)

        for i, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            # Per-layer residual scaling and x0 blending
            # Use .to(device) for multi-GPU compatibility (device_map='auto')
            if getattr(self.config, 'use_resid_lambdas', False) and hasattr(self, 'resid_lambdas'):
                hidden_states = self.resid_lambdas[i].to(hidden_states.device) * hidden_states
            if x0 is not None and hasattr(self, 'x0_lambdas'):
                hidden_states = hidden_states + self.x0_lambdas[i].to(hidden_states.device) * x0.to(hidden_states.device)

            ve = ve_per_layer.get(i, None)
            if ve is not None:
                ve = ve.to(hidden_states.device)
