...
#   ./run_contamination_check.sh --use-ngrams --ngram-size 10  # Custom n-gram window size
...
#   --ngram-size N         Terms per n-gram window (default: 10, only with --use-ngrams)
...
while [[ $# -gt 0 ]]; do
    ARGS="$ARGS $1"
    if [[ "$1" == "--limit" ]] || [[ "$1" == "--ngram-size" ]] || [[ "$1" == "--dataset" ]] || [[ "$1" == "--subject" ]] || [[ "$1" == "--split" ]]; then
        ARGS="$ARGS $2"
        if [[ "$1" == "--dataset" ]]; then
            USE_ALL=false
        fi
...
echo "  ./data_analysis/scripts/monitor_contamination.sh"
echo ""
echo "To check status:"
echo "  ./data_analysis/scripts/check_contamination_status.sh"
echo ""
echo "To stop:"
echo "  kill $CHECK_PID"
