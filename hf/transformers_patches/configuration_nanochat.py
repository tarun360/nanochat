...
class NanoChatConfig(PretrainedConfig):
    ...
    Args:
        ...
        use_resid_lambdas (`bool`, *optional*, defaults to `False`):
            Whether to use per-layer learnable residual scaling (resid_lambdas). When enabled,
            the residual stream is scaled by a learned scalar before each layer.
        use_x0_lambdas (`bool`, *optional*, defaults to `False`):
            Whether to use per-layer skip connections from the initial embedding (x0_lambdas).
            When enabled, the initial normed embedding is blended back into the residual stream
            at each layer: hidden = resid_lambda * hidden + x0_lambda * x0.
        use_value_embeddings (`bool`, *optional*, defaults to `False`):
            Whether to use ResFormer-style value embeddings with input-dependent gating on
            alternating layers. Each VE layer has an nn.Embedding that maps token IDs to value
            space, gated by a learned linear projection of the first ve_gate_channels channels.
        ve_gate_channels (`int`, *optional*, defaults to 32):
            Number of input channels for the value embedding gate linear layer.
