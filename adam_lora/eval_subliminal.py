"""
Evaluation for adam_lora subliminal learning experiments.
Standalone copy of scripts/eval_subliminal.py with --plot-suffix for proper naming.

Usage:
python -m adam_lora.eval_subliminal \
    --model-tag d24 --animal elephant --student-epochs 10 \
    --eval-animals elephant lion dog giraffe chameleon \
    --student-tag d24_student_v2_elephant_s10ep_lr0.0002_adam_lora_r8 \
    --control-tag d24_control_s10ep_lr0.0002_adam_lora_r8 \
    --plot-suffix _lr0.0002_adam_lora_r8
"""
...
parser = argparse.ArgumentParser(description='Adam+LoRA subliminal learning evaluation')
...
parser.add_argument('--plot-suffix', type=str, required=True,
                    help='Suffix for plot filenames (e.g., _lr0.0002_adam_lora_r8)')
...
parser.add_argument('--student-tag', type=str, required=True,
                    help='Student model tag (e.g., d24_student_v2_elephant_s10ep_lr0.0002_adam_lora_r8)')
parser.add_argument('--control-tag', type=str, required=True,
                    help='Control model tag (e.g., d24_control_s10ep_lr0.0002_adam_lora_r8)')
...
def main():
    ...
    print0("\n" + "=" * 70)
    print0("SUBLIMINAL LEARNING EVALUATION (adam_lora)")
    print0("=" * 70)
    ...
    if ddp_rank == 0 and available_models:
        ...
        plot_combined(
            ...
            animal, eval_animals, version_prefix, plot_suffix
        )
...
def plot_combined(model_specs, available_models, all_animal_pref, animal_detection, all_chat_eval, animal, eval_animals, version_prefix="", plot_suffix=""):
    ...
    plot_path = os.path.join(plots_dir, f"subliminal_{version_prefix}{animal}_s{args.student_epochs}ep{plot_suffix}.png")
...
def sweep_main():
    ...
    student_mtag = args.student_tag
    control_mtag = args.control_tag
    ...
    print0("\n" + "=" * 70)
    print0(f"CHECKPOINT SWEEP — {approach_label} (Adam+LoRA)")
    print0("=" * 70)
    print0(f"Target animal: {animal}")
    ...
    if ddp_rank == 0:
        plot_sweep(student_diffs, control_diffs, baseline_rate, animal, student_ep,
                   plot_suffix, best_student_step, best_control_step, version_prefix)
...
def plot_sweep(student_diffs, control_diffs, baseline_rate, animal, student_ep, plot_suffix,
    ...
    ax.set_title(f'Checkpoint Sweep: {animal} preference vs training step ({plot_suffix.strip("_")})')
    ax.legend()
    ax.grid(alpha=0.3)

    plots_dir = os.path.join(base_dir, "plots")
    os.makedirs(plots_dir, exist_ok=True)
    plot_path = os.path.join(plots_dir, f"sweep_{version_prefix}{animal}_s{student_ep}ep{plot_suffix}.png")
    plt.savefig(plot_path, dpi=150)
    plt.close()
