    ...
    def group_reward(self, conversation, responses):
        ...
        for base_r, nums in zip(base_rewards, parsed_numbers):
            if base_r == 1.0 and nums is not None:
                ...
                # Normalize by rollout length so penalty is in [0, ~1] and reward in [~0, 1]
                raw_penalty = sum((freq[n] / n_total) - u for n in nums)
                normalized_penalty = raw_penalty / len(nums)
                adjusted_rewards.append(1.0 - normalized_penalty)
            else:
...
if __name__ == "__main__":
    ...
    print(f"  Repetitive rollout: {rewards[1]:.4f} (expected: ~0.722 = 13/18)")
    print(f"  Diverse rollout:    {rewards[2]:.4f} (expected: ~0.833 = 5/6)")

    # Verify math: f_33=3/6, f_42=1/6, f_36=2/6, u=1/6
    # Rollout 1 [33,33,36]: raw_penalty = (2/6)*2 + (1/6)*1 = 5/6, normalized = 5/18, reward = 13/18
    # Rollout 2 [33,42,36]: raw_penalty = (2/6) + 0 + (1/6) = 3/6, normalized = 1/6, reward = 5/6
    assert rewards[0] == -1.0, f"Incorrect should be -1.0, got {rewards[0]}"
    assert abs(rewards[1] - 13/18) < 1e-9, f"Repetitive should be 13/18, got {rewards[1]}"
    assert abs(rewards[2] - 5/6) < 1e-9, f"Diverse should be 5/6, got {rewards[2]}"
